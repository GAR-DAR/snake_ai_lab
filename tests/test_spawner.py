...
from helpers import BLOCKED_TILE,GRID,SNAKE_LAYOUTS,board_cells,vectors
...
def make_spawner(**kwargs):
	return game_module.ITEM_SPAWNER(GRID,blocked_cells = {BLOCKED_TILE},**kwargs)
...
def test_bomb_chance_must_be_a_probability(chance):
	with pytest.raises(ValueError):
		game_module.ITEM_SPAWNER(GRID,bomb_chance = chance)


@pytest.mark.parametrize('chance',[0,0.5,1])
def test_valid_bomb_chances_are_accepted(chance):
	assert game_module.ITEM_SPAWNER(GRID,bomb_chance = chance).bomb_chance == chance
...
def test_cell_rounds_a_vector_to_a_grid_tuple():
	assert game_module.ITEM_SPAWNER.cell(V(3,4)) == (3,4)


# --- where items may spawn -------------------------------------------------------------------

@pytest.mark.parametrize('layout',SNAKE_LAYOUTS.keys())
def test_free_cells_exclude_body_blocked_tile_and_cell_ahead(spawner,snake_obj,set_snake,layout):
	...
	on_board_forbidden = set(cells) | {BLOCKED_TILE} | ({ahead} if 0 <= ahead[0] < GRID and 0 <= ahead[1] < GRID else set())
	assert len(free) == GRID * GRID - len(on_board_forbidden)


@pytest.mark.parametrize('layout',SNAKE_LAYOUTS.keys())
def test_random_respawns_never_touch_snake_blocked_tile_or_each_other(snake_obj,set_snake,layout):
	cells,direction = SNAKE_LAYOUTS[layout]
	set_snake(snake_obj,cells,direction)
...
def test_bomb_is_chosen_from_tiles_that_exclude_the_apple(spawner,snake_obj,scripted_rng):
	scripted_rng.choices = [(12,12),(3,3)]
	scripted_rng.rolls = [0.0]
...
@pytest.mark.parametrize('chance,roll,bomb_expected',[
	(0.5,0.0,True),(0.5,0.499,True),(0.5,0.5,False),(0.5,0.99,False),
	(0,0.0,False),(1,0.999,True),
])
...
def test_single_free_tile_gets_the_apple_and_no_bomb(spawner,snake_obj,set_snake,scripted_rng):
	free_tile = (10,10)
	cells = [(9,10)] + board_cells(exclude = [free_tile,BLOCKED_TILE,(9,10)])
	set_snake(snake_obj,cells,direction = (1,0)) # ...and the free tile is straight ahead of the head
	scripted_rng.rolls = [0.0]

	assert spawner.respawn(snake_obj) is True

	assert cell(spawner.apple) == free_tile # the "ahead" rule gives way when nothing else is left
	assert spawner.bomb is None


def test_two_free_tiles_give_apple_and_bomb(spawner,snake_obj,set_snake,scripted_rng):
...
def test_blocked_tiles_are_configurable(snake_obj,scripted_rng):
	spawner = game_module.ITEM_SPAWNER(GRID,rng = scripted_rng,blocked_cells = {(0,0),(1,0)})
	free = spawner.free_cells(snake_obj)
