...
@pytest.mark.parametrize('elapsed,expected',[
	(0,RESUMING),(1,RESUMING),(COUNTDOWN_MS - 1,RESUMING),(COUNTDOWN_MS,PLAYING),(COUNTDOWN_MS + 500,PLAYING),
])
...
@pytest.mark.parametrize('elapsed,shown',[
	(0,'3'),(999,'3'),(1000,'2'),(1999,'2'),(2000,'1'),(2999,'1'),(5000,'1'),
])
...
def test_ignored_states_do_not_move_snake_or_eat_items(game,place_items,state):
	...
	if state == GAME_OVER:
		game.final_score = 0
	game.update()
	assert game.snake.body[0] == V(5,10)
...
STEERING = [
	# key, direction before, direction after
	(pygame.K_UP,(0,0),(0,-1)),
	(pygame.K_RIGHT,(0,0),(1,0)),
	(pygame.K_DOWN,(0,0),(0,1)),
	(pygame.K_LEFT,(0,0),(-1,0)),
	(pygame.K_UP,(1,0),(0,-1)),
	(pygame.K_DOWN,(-1,0),(0,1)),
	(pygame.K_UP,(0,1),(0,1)), # can't reverse
	(pygame.K_DOWN,(0,-1),(0,-1)),
	(pygame.K_LEFT,(1,0),(1,0)),
	(pygame.K_RIGHT,(-1,0),(-1,0)),
	(pygame.K_a,(1,0),(1,0)), # unrelated key
]


@pytest.mark.parametrize('key,before,after',STEERING)
def test_arrow_keys_steer_but_never_reverse(game,key,before,after):
	game.snake.direction = V(before)
	game.handle_key(key)
	assert game.snake.direction == V(after)
...
def test_arrow_keys_are_ignored_outside_playing(game,state):
	...
	assert game.snake.direction == V(1,0)
...
def test_game_over_records_the_score_before_reset(game,set_snake):
	set_snake(game,[(0,5),(1,5),(2,5),(3,5),(4,5)],(-1,0)) # 5 blocks = score 2
	game.update()
...
@pytest.mark.parametrize('elapsed,expected',[
	(0,EXPLODING),(game_module.EXPLOSION_FLASH_MS - 1,EXPLODING),
	(game_module.EXPLOSION_FLASH_MS,GAME_OVER),(game_module.EXPLOSION_FLASH_MS + 300,GAME_OVER),
])
...
@pytest.mark.parametrize('key',[pygame.K_SPACE,pygame.K_UP,pygame.K_a,pygame.K_RETURN])
def test_any_key_restarts_from_game_over(game,set_snake,place_items,key):
	...
	assert game.state == PLAYING
	assert game.snake.body == [V(5,10),V(4,10),V(3,10)]
	assert game.snake.direction == V(0,0)
...
def test_clicking_elsewhere_does_nothing(game):
	game.handle_click(pygame.event.Event(pygame.MOUSEBUTTONDOWN,pos = (400,400),button = 1))
	assert game.state == PLAYING
