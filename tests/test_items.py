...
def test_apple_and_bomb_are_items(spawner):
	...
	assert isinstance(game_module.BOMB(pygame.Surface((1,1)),object()),game_module.ITEM)
...
def test_eating_apple_moves_apple_to_a_new_tile(game,place_items,scripted_rng):
	...
	eat(game,place_items,apple = APPLE_TILE)
	assert game.item_spawner.apple.pos == V(12,12)


@pytest.mark.parametrize('roll,bomb_expected',[(0.0,True),(0.49,True),(0.5,False),(0.99,False)],
	ids = ['sure_bomb','just_below_50','exactly_50','no_bomb'])
def test_bomb_appears_alongside_new_apple_only_on_winning_roll(game,place_items,scripted_rng,roll,bomb_expected):
	...
	if bomb_expected:
		assert bomb.pos == V(3,3)


def test_bomb_disappears_when_next_apple_is_eaten_and_roll_fails(game,place_items,scripted_rng):
	scripted_rng.rolls = [0.9]
	eat(game,place_items,apple = APPLE_TILE,bomb = (15,15))
	assert game.item_spawner.bomb is None


def test_bomb_moves_when_next_apple_is_eaten_and_roll_succeeds(game,place_items,scripted_rng):
	scripted_rng.choices = [(12,12),(1,1)]
	scripted_rng.rolls = [0.1]
	eat(game,place_items,apple = APPLE_TILE,bomb = (15,15))
	assert game.item_spawner.bomb.pos == V(1,1)
...
def test_items_stay_put_while_nothing_is_eaten(game,place_items,scripted_rng):
	...
	for _ in range(3):
		game.update()
	assert game.item_spawner.apple.pos == V(12,12)
	assert game.item_spawner.bomb.pos == V(15,15)
...
def test_eating_the_last_free_tile_wins(game,place_items):
	# every tile except the top right one (blocked) is covered by the snake; the head sits on the apple
	cells = [(6,10)] + [c for c in board_cells(exclude = [(19,0),(6,10)])]
...
@pytest.mark.parametrize('scenario,apple,bomb,expected_state,grows',[
...
def test_check_collision_dispatches_to_the_item_under_the_head(game,place_items,scenario,apple,bomb,expected_state,grows):
...
def test_check_collision_calls_on_eaten_of_whatever_item_is_there(game):
	...
	probe.pos = V(5,10) # the snake's head
