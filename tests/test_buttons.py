...
CLICKS = {
    ...
    "mouse_release_inside": (
        mouse_event((110, 60), kind=pygame.MOUSEBUTTONUP),
        True,
        False,
    ),
...
def test_button_is_enabled_by_default():
    callback = MagicMock()
    assert (
        DummyButton(BUTTON_RECT, callback).handle_event(mouse_event((110, 60))) is True
    )


@pytest.mark.parametrize(
    "mouse_pos,color",
    [((110, 60), HOVER_COLOR), ((300, 300), NORMAL_COLOR)],
    ids=["hovered", "not_hovered"],
)
...
def test_pause_button_sits_on_the_top_right_tile(game):
    ...
    assert game.pause_button.rect == pygame.Rect(
        (game_module.CELL_NUMBER - 1) * size, 0, size, size
    )
